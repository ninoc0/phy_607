...
def plot_scattering_angle_vs_b(N=20):
    ...
    plt.plot(
        xvals,
        linear(xvals, regression_sol.slope, regression_sol.intercept),
        color="red",
    )
    plt.text(
        0.2e-12, 30, rf"$R^2$ = {np.round(regression_sol.rvalue**2,5)}", fontsize=14
    )
...
def plot_scattering_angles(N=100):
    ...
    for b in impact_params:
        ...
        ):  # allows for random sign flip, demonstration of angular symmetry of problem
            b = -b
...
def plot_scattering_energy():
    ...
    regression_sol = linregress(
        energies / (e_charge * 1e6), 1 / np.tan(np.array(angles) / 2)
    )
    ...
    plt.scatter(
        energies / (e_charge * 1e6),
        1 / np.tan(np.array(angles) / 2),
        label="Data",
        marker="^",
    )
    plt.plot(
        x_vals / (e_charge * 1e6),
        linear(
            x_vals / (e_charge * 1e6), regression_sol.slope, regression_sol.intercept
        ),
        color="red",
        alpha=0.5,
...
def plot_atomic_numbers():
    ...
    for i, c in zip(nuclei, Z):
        if k % 2 == 0:
            plt.text(
                1 / (Z[k]),
                1 / np.tan(angles_Z[k] / 2) - 0.5,
                rf"${i}^{{{c}}}$",
                fontsize=12,
            )
        else:
            plt.text(
                1 / (Z[k]), 1 / np.tan(angles_Z[k] / 2), rf"${i}^{{{c}}}$", fontsize=12
            )
        k += 1
    ...
    plt.text(
        1 / Z[2],
        42.5,
        rf"$R^2$ = {np.round(regression_sol_Z.rvalue**2,5)}",
        fontsize=14,
    )
    plt.plot(
        x_vals_Z,
        linear(x_vals_Z, regression_sol_Z.slope, regression_sol_Z.intercept),
        color="red",
        alpha=0.5,
        label="Line of Best Fit",
    )
...
def time_simulation(N):
    """Simulate trajectories for N particles in order to benchmark algorithm"""
    overall_tic = time.time()
    
    power_samp_tic = time.time()
    impact_params = s.sampling.sample_power_law(N, a=0.5) * 1e-12
    power_samp_toc = time.time()

    gaussian_tic = time.time()
    energy = s.sampling.sample_gaussian(N, mu = 0.5, sigma = 0.1) * E_mean_joules
    gaussian_toc = time.time()
    
    traj_tic = time.time()

    for i in range(len(impact_params)):
        b = impact_params[i]
        v0 = np.sqrt((2 * energy[i]) / m_alpha)
    ...
    overall, power_samp, gaussian, traj = overall_toc - overall_tic, power_samp_toc - power_samp_tic, gaussian_toc - gaussian_tic, traj_toc - traj_tic

    return (overall, power_samp, gaussian, traj)
...
def plot_benchmark():
    ...
    for n in N:
        times.append(time_simulation(n)[0]) #append overall time to execute algorithm
    
    plt.figure(figsize=(8,6))
    plt.scatter(N, times, marker = '^')
    plt.grid(alpha = 0.5)
    plt.xlabel("Number of Particles", fontsize = 14)
    plt.ylabel("Execution Time [s]", fontsize = 14)
    plt.title("Time Complexity of Simulation Algorithm", fontsize = 14)
    plt.savefig("timecomplexity.png", dpi=300)

def plot_time_bar():
    N = 50

    overall, power_samp, gaussian, traj = time_simulation(N)
    power_samp_frac, gaussian_frac, traj_frac = power_samp/overall, gaussian/overall, traj/overall

    labels = ['Power Law Sampling', 'Gaussian Sampling', 'Solving Trajectory']
    sizes = [power_samp_frac, gaussian_frac, traj_frac]
    
    plt.figure(figsize=(8,6))
    plt.bar(labels, sizes)
    plt.ylabel("Percentage of Execution Time", fontsize = 14)
    plt.title("Time Spent in Each Sub-Routine, N=50", fontsize = 14)
    plt.savefig("time_subroutine.png", dpi=300)
